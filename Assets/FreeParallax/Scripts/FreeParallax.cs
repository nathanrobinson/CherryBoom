...
public class FreeParallax : MonoBehaviour
{
    ...
    private void SetupElementAtIndex(int i)
    {
        ...
        foreach (var obj in e.GameObjects)
        {
            if (obj == null)
            {
    ...
    {
        var pos = new Vector3(x, y, obj.transform.position.z);
        obj.transform.position = pos;

        var xOffset = r.bounds.min.x - obj.transform.position.x;
        if (xOffset != 0)
        {
            pos.x -= xOffset;
            obj.transform.position = pos;
        }
        var yOffset = r.bounds.min.y - obj.transform.position.y;
        if (yOffset != 0)
        {
            pos.y -= yOffset;
            obj.transform.position = pos;
...
public class FreeParallaxElement
{
    ...
    public void SetupState(FreeParallax p, Camera c, int index)
    {
        ...
        if (RepositionLogic.PositionMode != FreeParallaxPositionMode.IndividualStartOffScreen &&
            ...
            GameObjects.Count == 1)
        {
            var obj = Object.Instantiate(GameObjects[0]);
    ...
    {
        var worldBottom = c.ViewportToWorldPoint(Vector3.zero);

        for (var i = 0; i < GameObjects.Count; i++)
        {
            var obj = GameObjects[i];
            var r = GameObjectRenderers[i];
            var b = r.bounds;
            ...
            if (RepositionLogic.ScaleHeight > 0.0f)
            {
                ...
                if (p.IsHorizontal)
                {
                    var maxPoint = c.WorldToViewportPoint(new Vector3(0.0f, worldBottom.y + b.size.y, 0.0f));
                    percent = RepositionLogic.ScaleHeight / maxPoint.y;
                }
                else
                {
                    var maxPoint = c.WorldToViewportPoint(new Vector3(worldBottom.x + b.size.x, 0.0f, 0.0f));
                ...
                SpeedRatio != 0.0f)
            {
                ...
                else
                {
                    ...
                    var objHeight = c.WorldToViewportPoint(new Vector3(0.0f, worldBottom.y + b.size.y, 0.0f)).y;
                    if (objHeight < 1.1f)
                    {
    ...
    {
        var screenLeft = c.ViewportToWorldPoint(Vector3.zero);
        var screenTop = c.ViewportToWorldPoint(Vector3.one);
        float start, offset;
        ...
        for (var i = 0; i < GameObjects.Count; i++)
        {
            ...
            else
            {
                ...
                if (RepositionLogic.PositionMode == FreeParallaxPositionMode.WrapAnchorTop)
                {
                    if (p.IsHorizontal)
                    {
                        var topWorld = c.ViewportToWorldPoint(new Vector3(0.0f, 1.0f, 0.0f));
                        FreeParallax.SetPosition(obj, r, offset, topWorld.y - r.bounds.size.y);
                    }
                    else
                    {
                        var topWorld = c.ViewportToWorldPoint(new Vector3(1.0f, 0.0f, 0.0f));
    ...
    public void Update(FreeParallax p, float t, Camera c)
    {
        ...
        var wrap = RepositionLogic.PositionMode != FreeParallaxPositionMode.IndividualStartOffScreen &&
            RepositionLogic.PositionMode != FreeParallaxPositionMode.IndividualStartOnScreen;

        // if it's an individual object we let it go an extra screen width in case the player changes direction
        var padding = (wrap ? 0.0f : 1.0f);
        ...
        for (var i = 0; i < end; i++)
        {
            ...
            var r = GameObjectRenderers[i];
            var b = r.bounds;
            var screenEdge = (t > 0 ? c.WorldToViewportPoint(b.min) : c.WorldToViewportPoint(b.max));
            var screenEdgeValue = (p.IsHorizontal ? screenEdge.x : screenEdge.y);
            ...
            if (wrap)
            {
                if (t > 0 && screenEdgeValue >= maxEdge)
                {
                    if (p.IsHorizontal)
                    {
                        ...
                        var newX = (GameObjectRenderers[0].bounds.min.x - r.bounds.size.x) + p.WrapOverlap;
                        FreeParallax.SetPosition(obj, r, newX, r.bounds.min.y);
                    }
                    else
                    {
                        // move to the back of the line at far bottom
                        var newY = (GameObjectRenderers[0].bounds.min.y - r.bounds.size.y) + p.WrapOverlap;
                ...
                else if (t < 0 && screenEdgeValue <= minEdge)
                {
                    if (p.IsHorizontal)
                    {
                        ...
                        var newX = (GameObjectRenderers[GameObjects.Count - 1].bounds.max.x) - p.WrapOverlap;
                        FreeParallax.SetPosition(obj, r, newX, r.bounds.min.y);
                    }
                    else
                    {
                        // move to the front of the line at far top
                        var newY = (GameObjectRenderers[GameObjects.Count - 1].bounds.max.y) - p.WrapOverlap;
                        FreeParallax.SetPosition(obj, r, r.bounds.min.x, newY);
                    }
            ...
            else if (p.IsHorizontal)
            {
                if (t > 0 && (screenEdge.y >= c.rect.height || screenEdgeValue >= maxEdge))
                {
                    ...
                    {
                        var leftEdge = c.ViewportToWorldPoint(Vector3.zero);
                        var randX = UnityEngine.Random.Range(RepositionLogic.MinXPercent, RepositionLogic.MaxXPercent);
                        var randY = UnityEngine.Random.Range(RepositionLogic.MinYPercent, RepositionLogic.MaxYPercent);
                        var newWorldPoint = c.ViewportToWorldPoint(new Vector3(randX, randY));
                ...
                else if (t < 0 && (screenEdge.y >= c.rect.height || screenEdge.x < minEdge))
                {
                    ...
                    {
                        var rightEdge = c.ViewportToWorldPoint(Vector3.one);
                        var randX = UnityEngine.Random.Range(RepositionLogic.MinXPercent, RepositionLogic.MaxXPercent);
                        var randY = UnityEngine.Random.Range(RepositionLogic.MinYPercent, RepositionLogic.MaxYPercent);
                        var newWorldPoint = c.ViewportToWorldPoint(new Vector3(randX, randY));
            ...
            else
            {
                if (t > 0 && (screenEdge.x >= c.rect.width || screenEdgeValue >= maxEdge))
                {
                    ...
                    {
                        var bottomEdge = c.ViewportToWorldPoint(Vector3.zero);
                        var randX = UnityEngine.Random.Range(RepositionLogic.MinXPercent, RepositionLogic.MaxXPercent);
                        var randY = UnityEngine.Random.Range(RepositionLogic.MinYPercent, RepositionLogic.MaxYPercent);
                        var newWorldPoint = c.ViewportToWorldPoint(new Vector3(randX, randY));
                ...
                else if (t < 0 && (screenEdge.x >= c.rect.width || screenEdge.y < minEdge))
                {
                    ...
                    {
                        var topEdge = c.ViewportToWorldPoint(Vector3.one);
                        var randX = UnityEngine.Random.Range(RepositionLogic.MinXPercent, RepositionLogic.MaxXPercent);
                        var randY = UnityEngine.Random.Range(RepositionLogic.MinYPercent, RepositionLogic.MaxYPercent);
                        var newWorldPoint = c.ViewportToWorldPoint(new Vector3(randX, randY));
